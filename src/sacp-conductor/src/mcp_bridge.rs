...
pub async fn run_mcp_bridge(port: u16) -> Result<(), agent_client_protocol_schema::Error> {
...
async fn connect_with_retry(port: u16) -> Result<TcpStream, agent_client_protocol_schema::Error> {
    let max_retries = 10;
    ...
    for attempt in 1..=max_retries {
        match TcpStream::connect(format!("127.0.0.1:{}", port)).await {
            ...
            Err(e) => {
                return Err(agent_client_protocol_schema::Error::into_internal_error(e));
            }
        }
    }
