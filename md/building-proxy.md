...
### Peers: AgentRole and ClientRole

Proxies communicate with two peers:

- **`AgentRole`** - The downstream direction (toward the AI agent)
- **`ClientRole`** - The upstream direction (toward the editor)

When you receive a message, you typically forward it (possibly modified) to the other peer:

```rust
use sacp::{AgentRole, ClientRole};

// Message from client -> forward to agent
cx.send_request_to(AgentRole, modified_request)?;

// Message from agent -> forward to client  
cx.send_notification_to(ClientRole, modified_notification)?;
```
...
ProxyToConductor::builder()
    ...
    .on_receive_notification_from(AgentRole, handler, sacp::on_receive_notification!())
    .connect_to(transport)?
    .serve()
    .await
```
...
// From agent
.on_receive_request_from(AgentRole, handler, macro!())
.on_receive_notification_from(AgentRole, handler, macro!())
```
